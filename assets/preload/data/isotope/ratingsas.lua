...
function onCreatePost()
	...
	precacheImage('vanilla/shit');
	precacheImage('vanilla/bad');
	precacheImage('vanilla/good');
	precacheImage('vanilla/sick');
	precacheImage('vanilla/num');
end	
...
function goodNoteHit(id, direction, noteType, isSustainNote)
	if not isSustainNote and resetHideHud and getProperty('combo') then
		...
		makeLuaSprite('lastRating', 'vanilla/' .. rating, screenWidth * 0.35 - -42 + downscrolloffset + 680 , screenHeight / 2 - 120 - downscrolloffset * 3 + 200 );
		...
		if combo >= 0 then
			for i=2,0,-1 do
				...
				makeLuaSprite(tag, 'vanilla/num' .. math.floor(combo / 10 ^ i % 10) , screenWidth * 0.35 - -105 - i * 43 + downscrolloffset + 680, screenHeight / 2 + -0 - downscrolloffset * 3 + 200 );
